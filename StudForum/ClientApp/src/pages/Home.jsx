                ...
                <Row sm="1" md="2" xl="3">
                    {
                        ...
                        ?
                        discussionPopList.map(d => <Col className="mt-4" key={`${d.id}`}><CardDiscussion k={`${d.id}`} title={d.title} text={d.content} date={`${new Date(d.createdDate).getDate()}.${new Date(d.createdDate).getMonth()}.${new Date(d.createdDate).getFullYear()} ${new Date(d.createdDate).getHours()}:${new Date(d.createdDate).getMinutes()}` }/> </Col>)
                        :
                        <></>
                    }
